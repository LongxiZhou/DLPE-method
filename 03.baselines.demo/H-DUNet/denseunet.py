...
def DenseUNet(nb_dense_block=4,
    ...
    if K.common.image_dim_ordering() == 'tf':
      concat_axis = 3
      img_input = Input(shape=(img_deps, img_rows, 3), name='data')
    else:
      concat_axis = 1
      img_input = Input(shape=(3, img_deps, img_rows), name='data')
    ...
    for block_idx in range(nb_dense_block - 1):
        stage = block_idx+2
        x, nb_filter = dense_block(x, concat_axis, stage, nb_layers[block_idx], nb_filter, growth_rate, dropout_rate=dropout_rate, weight_decay=weight_decay)
        box.append(x)
        # Add transition_block
        x = transition_block(x, concat_axis, stage, nb_filter, compression=compression, dropout_rate=dropout_rate, weight_decay=weight_decay)
        nb_filter = int(nb_filter * compression)

    final_stage = stage + 1
    x, nb_filter = dense_block(x, concat_axis, final_stage, nb_layers[-1], nb_filter, growth_rate, dropout_rate=dropout_rate, weight_decay=weight_decay)
...
def conv_block(x, concat_axis,stage, branch, nb_filter, dropout_rate=None, weight_decay=1e-4):
...
def transition_block(x, concat_axis, stage, nb_filter, compression=1.0, dropout_rate=None, weight_decay=1E-4):
...
def dense_block(x, concat_axis, stage, nb_layers, nb_filter, growth_rate, dropout_rate=None, weight_decay=1e-4, grow_nb_filters=True):
    ...
    for i in range(nb_layers):
        branch = i+1
        x = conv_block(concat_feat, concat_axis, stage, branch, growth_rate, dropout_rate, weight_decay)
