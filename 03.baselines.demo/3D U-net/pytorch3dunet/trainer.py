...
class UNet3DTrainer:
    ...
    def train(self, train_loader):
        ...
        # all_count = 0
        # ratio_ls = []
        # logger.info(f'len_train_loader:{len(train_loader)}')
        # for i, t in enumerate(train_loader):
        #     input, target, weight = self._split_training_batch(train_loader[i])
        #     sum = torch.sum(target)
        #     sub_count = 1.0 / (sum.float() + 1.0)
        #     all_count = all_count + sub_count
        #     ratio_ls.append(sub_count)
        #     logger.info(f'iteration:{i}.  sub_count:{sub_count}')
        # ratio_ls = np.array(ratio_ls)
        # ratio_ls = ratio_ls / np.sum(ratio_ls)
        # logger.info(f'all_count:{all_count}')
        # logger.info(f'list_of_counts:{ratio_ls}')
        # for i in range(0,len(train_loader)):
        for i, t in enumerate(train_loader):
            #choose_num = np.random.choice(range(0,len(train_loader)),p = ratio_ls.ravel())
            #t = train_loader[choose_num]
            input, target, weight = self._split_training_batch(t)
            #print('len(trainloader):',len(train_loader))
            count = torch.cuda.device_count()
            sum = torch.sum(target)
            #class_weight = torch.tensor([1.0, count * 2097152.1 / (sum.int() + 1)]).to('cuda:0')
            logger.info(
                f'Training iteration {self.num_iterations}. Batch {i}. Epoch [{self.num_epoch}/{self.max_num_epochs - 1}].')

            if sum < count * 2500:
                logger.info(f'Neglect the non-infection part!!!! sum_target:{sum}. threshold:{count * 2500}')
                continue
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                self.writer.add_scalars('../train_val_loss',{'train_loss':train_losses.avg},self.num_iterations)
                self._log_params()
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            print('len(valid):',len(val_loader))
            ...
            self.writer.add_scalars('../train_val_loss',{'val_loss':val_losses.avg},self.num_iterations)
    ...
    def _forward_pass(self, input, target, weight=None):
        ...
        if weight is None:
            ...
            #logger.info(f'loss_weight: {loss}. loss_without_weight:{loss1}')
    ...
    def _log_stats(self, phase, loss_avg, eval_score_avg):
        ...
        self.writer.add_scalars('train_valid_loss',{f'{phase}':loss_avg},self.num_iterations)
